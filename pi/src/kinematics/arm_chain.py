...
Link lengths are straight from kit's diagram:
    BASE_TO_SHOULDER = 76.5 mm (base plate to shoulder pivot)
...
Coordinate frame: origin at base's rotation axis, at the top of the base plate. +Z is up. +X is horizontal reach direction when base = 90.
...
BASE_TO_SHOULDER = 76.5
SHOULDER_TO_ELBOW = 58
ELBOW_TO_GRIPPER = 75

def _bounds(servo_lo, servo_hi):
    """Convert a safe servo range (degress) to the chain's zero-at-90 radians."""
...
def solve_ik(x, y, z, initial_servo_pose=(90, 90, 90)):
    """
    Solve for the (base, shoulder, elbow) servo angles that place the gripper at the given (x, y, z) mm position.
    Only position, no orientation control
    """
